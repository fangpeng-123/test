...
embeddings = DashScopeEmbeddings(
    model="text-embeeding-v1",
    dashscope_api_key="sk-60f37b1aca9a4e78acde3bf2ac2f21cd",
)
...
def user_input(user_question):
    ...
    try:
        ...
        retriever = new_db.as_retriever()  # 将数据转化为 Langchain 检索工具
        retriever_chain = create_retriever_tool(retriever, "pdf_extractor",
        ...
        st.info("请重新处理 PDF 文件")

def get_conversational_chain(tools, querys):
    prompt = ChatPromptTemplate([
        (
            "system",
            """你是 AI 助手，请根据提供的上下文回答，确保提供所有细节，如果答案不在上下文中，请说"答案不再上下文中"，不要提供错误答案""",
        ),
...
def main():
    st.set_page_config("🤖 LangChain 知识库系统开发")
    st.header("🤖 LangChain 知识库系统开发")
